...
class Data_utility(object):
    ...
    def _split(self, train, valid, test):
        ...
        test_set  = pd.read_csv('/home/ubuntu/OpenMarsML/data/data_files/series_to_predict.csv')
        print(test_set)
    ...
    def get_batches(self, inputs, targets, batch_size, shuffle=True):
        length = len(inputs)
        print(f'length  of intput us {length}')
        if shuffle:
...
class Arguments():
    def __init__(self, data, hidCNN=100, hidRNN=100, window=35, CNN_kernel=6, highway_window=24, clip=10, epochs=5,
                 batch_size=96, dropout=0.2, save="save.pt", optim="adam", lr=0.001, horizon=1, skip=24, hidSkip=5,
...
args = Arguments(horizon=84, skip = 0, window = 84, hidCNN=30, hidRNN=30, L1loss=False, data=dataframe, save=f'{model_file}', output_fun=None,
...
Y  = pd.read_csv('/home/ubuntu/OpenMarsML/data/data_files/predicted_series_actual_vals.csv')

print(Data.test.)
predict = evaluate2(Data, Data.test, Y, model, evaluateL2, evaluateL1, args.batch_size)
